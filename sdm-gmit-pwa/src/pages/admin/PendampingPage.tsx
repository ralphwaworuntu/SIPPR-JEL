...
export default function PendampingPage() {
    ...
    const [selectedItem, setSelectedItem] = useState<Pendamping | null>(null);
    ...
    const { data: pendamping = [], isLoading } = useQuery<Pendamping[]>({
        queryKey: ['pendamping'],
        queryFn: async () => {
            const res = await axios.get(`${import.meta.env.VITE_API_URL || `http://${window.location.hostname}:3000`}/api/pendamping`, {
    ...
    const deleteMutation = useMutation({
        mutationFn: async (id: number) => {
            await axios.delete(`${import.meta.env.VITE_API_URL || `http://${window.location.hostname}:3000`}/api/pendamping/${id}`, {
                withCredentials: true
            });
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['pendamping'] });
    ...
    const addMutation = useMutation({
        mutationFn: async (data: any) => {
            await axios.post(`${import.meta.env.VITE_API_URL || `http://${window.location.hostname}:3000`}/api/pendamping`, data, {
                withCredentials: true
            });
        },
        onSuccess: () => {
            ...
            setFormData({ name: '', lingkungan: '', familyCount: 0, enumeratorCount: 0, whatsapp: '', loginEmail: '', loginPassword: '' });
        }
    });

    const handleOpenDetail = (item: Pendamping) => {
        setSelectedItem(item);
        setIsDetailModalOpen(true);
    };

    const handleOpenDelete = (item: Pendamping) => {
        setSelectedItem(item);
        setIsDeleteModalOpen(true);
    };

    const handleOpenCredential = (item: Pendamping) => {
        setSelectedItem(item);
        setCredentialData({
    ...
    return (
        <AdminLayout title="Kelola Pendamping Lingkungan">
            <div className="p-6 max-w-7xl mx-auto space-y-6">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        ...
                        <p className="text-slate-500 text-sm mt-1">Kelola data pendamping lingkungan</p>
                    </div>
                    <button
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm text-left">
                            <thead className="text-xs text-slate-500 uppercase bg-slate-50 dark:bg-slate-800/50 dark:text-slate-400">
                                <tr>
                                    <th className="px-6 py-4 font-medium">Nama Pendamping</th>
                                    <th className="px-6 py-4 font-medium">Lingkungan</th>
                                    <th className="px-6 py-4 font-medium">Jumlah Keluarga</th>
                                    <th className="px-6 py-4 font-medium">Jumlah Enumerator</th>
                            ...
                            <tbody className="divide-y divide-slate-200 dark:divide-slate-800/50">
                                {isLoading ? (
                                    ...
                                    <tr>
                                        <td colSpan={6} className="px-6 py-8 text-center text-slate-500">
                                            <div className="flex flex-col items-center justify-center gap-2">
                                                ...
                                                <p>Belum ada data pendamping lingkungan</p>
                                            </div>
                                        </td>
                                    </tr>
                                ) : pendamping.map((item) => (
                                    <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 text-slate-600 dark:text-slate-300">{item.familyCount}</td>
                                        <td className="px-6 py-4 text-slate-600 dark:text-slate-300">{item.enumeratorCount}</td>
                    ...
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm shadow-xl">
                        <div className="w-full max-w-lg bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col max-h-[90vh]">
                            <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center shrink-0">
                                <h3 className="text-lg font-bold text-slate-900 dark:text-white">Tambah Pendamping </h3>
                                <button
                            ...
                            <div className="p-6 space-y-4 overflow-y-auto">
                                <div>
                                    <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">Nama Pendamping *</label>
                                    <input
                                        type="text"
                                        value={formData.name}
                            ...
                            <div className="p-6 border-t border-slate-100 dark:border-slate-800 flex justify-end gap-3 shrink-0">
                                ...
                                <button
                                    ...
                                    disabled={addMutation.isPending || !formData.name || !formData.lingkungan}
                    ...
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
                        <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 overflow-hidden">
                            <div className="p-6 border-b border-slate-100 dark:border-slate-800 relative">
                                <h3 className="text-lg font-bold text-slate-900 dark:text-white">Detail Pendamping</h3>
                                <button
                            ...
                            <div className="p-6 space-y-4">
                                ...
                                <div className="grid grid-cols-3 gap-4 border-b border-slate-50 dark:border-slate-800/50 pb-4">
                                    <div className="text-sm text-slate-500">Jumlah Enum.</div>
                                    <div className="col-span-2 text-sm font-medium text-slate-900 dark:text-white">{selectedItem.enumeratorCount}</div>
                                </div>
                                <div className="grid grid-cols-3 gap-4 border-b border-slate-50 dark:border-slate-800/50 pb-4">
                ...
                {isDeleteModalOpen && selectedItem && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
                        <div className="w-full max-w-sm bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 p-6">
                            ...
                            <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2">Hapus Pendamping?</h3>
                            <p className="text-sm text-slate-500 mb-6">
                                Apakah Anda yakin ingin menghapus pendamping <strong>{selectedItem.name}</strong>? Tindakan ini tidak dapat dibatalkan.
                            </p>
