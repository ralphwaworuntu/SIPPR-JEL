...
const AdminMemberData = () => {
    ...
    const handleExportCSV = () => {
        const headers = [
            //Step 1 
            "Nomor Kartu Keluarga", "NIK", "Nama Lengkap Kepala Keluarga", "Jenis Kelamin", "Tanggal Lahir", "Usia", "Golongan Darah", "Status Baptis", "Status Sidi", "Status Pernikahan", "Pendidikan Terakhir", "Nomor Telepon/ WhatsApp Aktif", "Lingkungan", "Rayon", "Alamat Lengkap", "Kota/Kabupaten", "Kecamatan", "Kelurahan/Desa",
            //Step 2
        ...
        const rows = filteredMembers.map(m => {
            ...
            return [
                // Step 1
                esc(m.kkNumber), esc(m.nik), esc(m.name), esc(m.gender), esc(m.birthDate), calculateAge(m.birthDate), esc(m.bloodType), esc(m.baptismStatus), esc(m.sidiStatus), esc(m.maritalStatus), esc(m.educationLevel || '-'), esc(m.phone), esc(m.lingkungan), esc(m.rayon), esc(m.address), esc(m.city), esc(m.district), esc(m.subdistrict),
                // Step 2
