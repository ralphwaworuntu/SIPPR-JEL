...
const Step3Commitment: React.FC<StepProps> = ({ data, update }) => {
    ...
    useEffect(() => {
        // Only auto-initialize if they said "Ya" and list is empty
        if (data.willingnessToServe === 'Ya' && (!data.professionalFamilyMembers || data.professionalFamilyMembers.length === 0)) {
            update({
    ...
    }, [data.willingnessToServe, data.professionalFamilyMembers]);
