    ...
    return (
        <div className="space-y-6 animate-fadeIn">
            <div className="bg-white dark:bg-[#1a2e20] rounded-2xl shadow-sm border border-slate-100 dark:border-white/5 overflow-hidden transition-all duration-300">
                <div className="px-5 py-4 flex items-center gap-3 border-b border-slate-50 dark:border-white/5 bg-red-500 bg-opacity-10 dark:bg-opacity-20">
                    <div className="p-2 rounded-lg bg-red-500 text-white shadow-sm">
                        <span className="material-symbols-outlined text-[20px]">medical_services</span>
                    </div>
                    <h4 className="font-bold text-gray-800 dark:text-white text-base tracking-wide">Kondisi Kesehatan & Sosial</h4>
                </div>
                ...
                <div className="p-6 space-y-8">
                            ...
                            <FormRadioGroup
                                ...
                                onChange={(val) => {
                                    if (val === 'Ya') {
                                        ...
                                        update({ health_hasBPJS: val as 'Ya' | 'Tidak', health_bpjsNumber: '' });
                                    }
                                }}
                                columns={2}
                            />
                            ...
                            {data.health_hasBPJS === 'Ya' && (
                                <div className="animate-fadeIn pl-4 border-l-2 border-primary/20 pt-2">
                                    <label className="block text-sm font-semibold text-slate-700 dark:text-slate-200 mb-2">
                                        Nomor BPJS Aktif Kepala Keluarga <span className="text-red-500">*</span>
                                    </label>
                                    <textarea
                                        id="health_bpjsNumber"
                                        value={data.health_bpjsNumber || ''}
                                        onChange={(e) => update({ health_bpjsNumber: e.target.value })}
                                        ...
                                        placeholder="Masukkan nomor BPJS Kepala Keluarga..."
                                        required
                                    />
                                </div>
                            )}
                    ...
                    <div className="space-y-4">
                        ...
                        <FormRadioGroup
                            ...
                            onChange={(val) => {
                                update({ health_hasBPJSKetenagakerjaan: val });
                                if (val === 'Tidak') {
                                    update({ health_bpjsKetenagakerjaanProgram: '' });
                                }
                            }}
                            columns={2}
                        />

                        {data.health_hasBPJSKetenagakerjaan === 'Ya' && (
                            <div className="pl-4 border-l-2 border-primary/20 mt-4 animate-fadeIn">
                                <SectionHeader title="Program apa yang diikuti?" tooltipText="Pilih program BPJS Ketenagakerjaan yang diikuti saat ini." />
                                <FormSelect
                                    id="health_bpjsKetenagakerjaanProgram"
                                    options={['Jaminan Kematian (JKM)', 'Jaminan Kecelakaan Kerja (JKK)', 'Jaminan Hari Tua (JHT)', 'Tidak Semuanya']}
                                    value={data.health_bpjsKetenagakerjaanProgram}
                                    onChange={(val) => update({ health_bpjsKetenagakerjaanProgram: val })}
                    ...
                    {/* 7. Bansos */}
                    <div className="space-y-4">
                        <SectionHeader title="Keluarga Penerima Manfaat (KPM)" tooltipText="Apakah keluarga terdaftar sebagai penerima bantuan sosial dari pemerintah?" />
                        <FormRadioGroup
                            label="Apakah keluarga merupakan Keluarga Penerima Manfaat (KPM) Bantuan Sosial (Bansos) dari Pemerintah?"
                            id="health_isKPM"
                            name="health_isKPM"
                            options={['Ya', 'Tidak']}
                            value={data.health_isKPM || ''}
                            onChange={(val) => {
                                if (val === 'Ya') {
                                    update({ health_isKPM: val, health_isPoorNonKPM: '', health_poorNonKPMReason: '' });
                                } else {
                                    update({ health_isKPM: val, health_socialAssistance: '' });
                                }
                            }}
                            columns={2}
                        />

                        {data.health_isKPM === 'Ya' && (
                            <div className="animate-fadeIn pl-4 border-l-2 border-primary/20 pt-2">
                                <FormSelect
                                    label="Apa jenis bantuan sosial yang diterima?"
                                    id="health_socialAssistance"
                                    options={['PKH', 'BPNT', 'BLT', 'Bantuan Lainnya']}
                                    value={data.health_socialAssistance}
                                    onChange={(val) => update({ health_socialAssistance: val })}
                                    placeholder="Pilih Jenis Bansos..."
                                    required={true}
                                    tooltipText="Program bantuan sosial dari pemerintah yang rutin diterima."
                                />
                            </div>
                        )}

                        {data.health_isKPM === 'Tidak' && (
                            <div className="space-y-4 animate-fadeIn pl-4 border-l-2 border-primary/20 pt-2">
                                <FormRadioGroup
                                    label="Apakah Keluarga tergolong KK miskin extrim, KK miskin NAMUN TIDAK sebagai KPM Bansos?"
                                    id="health_isPoorNonKPM"
                                    name="health_isPoorNonKPM"
                                    options={['Ya', 'Tidak']}
                                    value={data.health_isPoorNonKPM || ''}
                                    onChange={(val) => {
                                        if (val === 'Tidak') {
                                            update({ health_isPoorNonKPM: val, health_poorNonKPMReason: '' });
                                        } else {
                                            update({ health_isPoorNonKPM: val });
                                        }
                                    }}
                                    columns={2}
                                />

                                {data.health_isPoorNonKPM === 'Ya' && (
                                    <div className="animate-fadeIn space-y-2">
                                        <label className="block text-sm font-semibold text-slate-700 dark:text-slate-200 mb-2">
                                            Jika Keluarga tergolong Miskin Ekstrim, KK Miskin namun tidak sebagai KPM Bansos, Mengapa? <span className="text-red-500">*</span>
                                        </label>
                                        <textarea
                                            id="health_poorNonKPMReason"
                                            value={data.health_poorNonKPMReason || ''}
                                            onChange={(e) => update({ health_poorNonKPMReason: e.target.value })}
                                            className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-primary/50 text-slate-900 dark:text-white min-h-[100px] resize-y font-medium text-sm"
                    ...
                    {/* Section Donor Darah */}
                    <div className="space-y-6 bg-blue-50/50 dark:bg-blue-950/10 p-5 rounded-2xl border border-blue-100 dark:border-blue-900/20">
                        <div className="space-y-4">
                            <SectionHeader title="Donor Darah" tooltipText="Kesediaan untuk membantu sesama melalui donor darah." />
                            <FormRadioGroup
                                label="Apakah Bersedia mendonorkan darahnya bagi sesama yang membutuhkan?"
                                id="health_willingToDonateBlood"
                                name="health_willingToDonateBlood"
                                options={['Ya', 'Tidak']}
                                value={data.health_willingToDonateBlood || ''}
                                onChange={(val) => {
                                    if (val === 'Tidak') {
                                        update({ health_willingToDonateBlood: val, health_willingToJoinBloodCommunity: '' });
                                    } else {
                                        update({ health_willingToDonateBlood: val });
                                    }
                                }}
                                columns={2}
                            />
                            ...
                            {data.health_willingToDonateBlood === 'Ya' && (
                                <div className="animate-fadeIn pl-4 border-l-2 border-primary/20 pt-2">
                                    <FormRadioGroup
                                        label="Apakah bersedia menjadi anggota Komunitas Donor Darah GMIT Emaus Liliba?"
                                        id="health_willingToJoinBloodCommunity"
                                        name="health_willingToJoinBloodCommunity"
                                        options={['Ya', 'Tidak']}
                                        value={data.health_willingToJoinBloodCommunity || ''}
                                        onChange={(val) => update({ health_willingToJoinBloodCommunity: val })}
                                        columns={2}
                                    />
                                </div>
                            )}
