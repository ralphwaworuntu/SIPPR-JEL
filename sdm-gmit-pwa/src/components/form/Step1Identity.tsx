...
const Step1Identity = ({ data, update }: StepProps) => {
    ...
    const handleBirthPlaceBlur = () => {
        if (!data.birthPlace) return;
        const formatted = data.birthPlace.toLowerCase().replace(/(?:^|\s)\S/g, function (a) { return a.toUpperCase(); });
        if (formatted !== data.birthPlace) {
            update({ birthPlace: formatted });
        }
    };

    const handleMarriagePlaceBlur = () => {
        if (!data.marriagePlace) return;
        const formatted = data.marriagePlace.toLowerCase().replace(/(?:^|\s)\S/g, function (a) { return a.toUpperCase(); });
        if (formatted !== data.marriagePlace) {
            update({ marriagePlace: formatted });
        }
    };

    const handleBaptismPlaceBlur = () => {
        if (!data.baptismPlace) return;
        const formatted = data.baptismPlace.toLowerCase().replace(/(?:^|\s)\S/g, function (a) { return a.toUpperCase(); });
        if (formatted !== data.baptismPlace) {
            update({ baptismPlace: formatted });
        }
    };

    const handleSidiPlaceBlur = () => {
        if (!data.sidiPlace) return;
        const formatted = data.sidiPlace.toLowerCase().replace(/(?:^|\s)\S/g, function (a) { return a.toUpperCase(); });
        if (formatted !== data.sidiPlace) {
            update({ sidiPlace: formatted });
        }
    };
    ...
    const [bapYear = '', bapMonth = '', bapDay = ''] = (data.baptismDate || '').split('-');

    const handleBapChange = (part: 'year' | 'month' | 'day', val: string) => {
        let newY = bapYear;
        let newM = bapMonth;
        let newD = bapDay;
        if (part === 'year') newY = val;
        if (part === 'month') newM = val;
        if (part === 'day') newD = val;
        update({ baptismDate: `${newY}-${newM}-${newD}` });
    };

    const [sidiYear = '', sidiMonth = '', sidiDay = ''] = (data.sidiDate || '').split('-');

    const handleSidiChange = (part: 'year' | 'month' | 'day', val: string) => {
        let newY = sidiYear;
        let newM = sidiMonth;
        let newD = sidiDay;
        if (part === 'year') newY = val;
        if (part === 'month') newM = val;
        if (part === 'day') newD = val;
        update({ sidiDate: `${newY}-${newM}-${newD}` });
    };
    ...
    return (
        <div className="space-y-6 animate-fadeIn">
            <div className="bg-white dark:bg-[#1a2e20] rounded-2xl shadow-sm border border-slate-100 dark:border-white/5 overflow-hidden transition-all duration-300">
                <div className="px-5 py-4 flex items-center gap-3 border-b border-slate-50 dark:border-white/5 bg-blue-500 bg-opacity-10 dark:bg-opacity-20">
                    <div className="p-2 rounded-lg bg-blue-500 text-white shadow-sm">
                        <span className="material-symbols-outlined text-[20px]">person</span>
                    </div>
                    <h4 className="font-bold text-gray-800 dark:text-white text-base tracking-wide">Data Umum Kepala Keluarga</h4>
                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="flex flex-col relative">
                            <FormInput
                                ...
                                tooltipText="Masukkan tempat lahir sesuai KTP."
                            />
                            {isValidating && (
                                <div className="absolute right-3 top-[3.2rem]">
                                    <span className="animate-spin h-4 w-4 border-2 border-primary border-t-transparent rounded-full block"></span>
                        ...
                        {data.baptismStatus === 'Sudah' && (
                            <>
                                <FormInput
                                    label="Tempat Baptis"
                                    id="baptismPlace"
                                    value={data.baptismPlace || ''}
                                    onChange={(val) => update({ baptismPlace: val })}
                                    onBlur={handleBaptismPlaceBlur}
                                    placeholder="Contoh: GMIT Emaus Liliba"
                                    required
                                    tooltipText="Masukkan nama tempat atau gereja melakukan baptisan."
                                />
                                <div className="flex flex-col gap-2 relative z-20">
                                    <label className="text-sm font-bold text-slate-800 dark:text-white flex items-center gap-1 z-10">
                                        Tanggal Baptis <span className="text-red-500">*</span>
                                        <FormTooltip text="Masukkan tanggal baptis." />
                                    </label>
                                    <div className="grid grid-cols-3 gap-2">
                                        <FormSelect
                                            id="bapDay"
                                            value={bapDay}
                                            onChange={(val) => handleBapChange('day', val)}
                                            options={daysOptions}
                                            placeholder="Tanggal"
                                            required
                                        />
                                        <FormSelect
                                            id="bapMonth"
                                            value={bapMonth}
                                            onChange={(val) => handleBapChange('month', val)}
                                            options={monthsOptions}
                                            placeholder="Bulan"
                                            required
                                        />
                                        <FormSelect
                                            id="bapYear"
                                            value={bapYear}
                                            onChange={(val) => handleBapChange('year', val)}
                                            options={yearsOptions}
                                            placeholder="Tahun"
                                            required
                                        />
                                ...
                                {data.sidiStatus === 'Sudah' && (
                                    <>
                                        <FormInput
                                            label="Tempat Sidi"
                                            id="sidiPlace"
                                            value={data.sidiPlace || ''}
                                            onChange={(val) => update({ sidiPlace: val })}
                                            onBlur={handleSidiPlaceBlur}
                                            placeholder="Contoh: GMIT Emaus Liliba"
                                            required
                                            tooltipText="Masukkan nama tempat atau gereja meneguhkan sidi."
                                        />
                                        <div className="flex flex-col gap-2 relative z-20">
                                            <label className="text-sm font-bold text-slate-800 dark:text-white flex items-center gap-1 z-10">
                                                Tanggal Sidi <span className="text-red-500">*</span>
                                                <FormTooltip text="Masukkan tanggal sidi." />
                                            </label>
                                            <div className="grid grid-cols-3 gap-2">
                                                <FormSelect
                                                    id="sidiDay"
                                                    value={sidiDay}
                                                    onChange={(val) => handleSidiChange('day', val)}
                                                    options={daysOptions}
                                                    placeholder="Tanggal"
                                                    required
                                                />
                                                <FormSelect
                                                    id="sidiMonth"
                                                    value={sidiMonth}
                                                    onChange={(val) => handleSidiChange('month', val)}
                                                    options={monthsOptions}
                                                    placeholder="Bulan"
                                                    required
                                                />
                                                <FormSelect
                                                    id="sidiYear"
                                                    value={sidiYear}
                                                    onChange={(val) => handleSidiChange('year', val)}
                                                    options={yearsOptions}
                                                    placeholder="Tahun"
                                                    required
                                                />
                        ...
                        <FormSelect
                            ...
                            options={['Belum Nikah', 'Sudah Nikah', 'Cerai Hidup', 'Cerai Mati', 'Tidak Menikah']}
                            ...
                            <>
                                {data.maritalStatus === 'Sudah Nikah' && (
                                    <FormInput
                                        ...
                                        onChange={(val) => update({ marriagePlace: val })}
                                        onBlur={handleMarriagePlaceBlur}
                                        placeholder="Contoh: Kupang"
                                        required
                                        tooltipText="Masukkan tempat mencatatkan pernikahan."
                                    />
                                )}
                                ...
                                <FormMultiSelect
                                    ...
                                    options={['Nikah Adat', 'Nikah Gereja', 'Nikah Catatan Sipil', 'Tidak Menikah']}
                        ...
                        <FormSelect
                            ...
                            options={['Tidak Sekolah', 'Tidak Tamat SD', 'SD', 'SMP/Sederajat', 'SMA/Sederajat', 'D I', 'D II', 'D III', 'D IV/S1', 'S1', 'S2', 'S3']}
