...
export const MemberDetailModal = ({ member, onClose, onEdit }: MemberDetailModalProps) => {
    ...
    return (
        <div className="flex flex-col animate-fade-in-up h-[85vh] md:h-auto md:max-h-[85vh]">

            {/* Header Passport Card - Shrinked */}
            <div className="sticky top-0 z-30 flex flex-row items-center gap-4 p-4 bg-gradient-to-br from-slate-900 to-slate-800 rounded-2xl border border-slate-700 relative overflow-hidden shadow-xl shrink-0 mx-1 mt-1 ring-1 ring-white/10">
                ...
                <div className="flex-1 z-10 w-full flex flex-col sm:flex-row justify-between sm:items-center gap-2">
                    <div>
                        ...
                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs font-medium text-slate-400">
                            <p className="font-mono tracking-wider">ID: {member.id}</p>
                            <span className="flex items-center gap-1"><span className="material-symbols-outlined text-[12px] text-primary">signpost</span> L{member.lingkungan}/R{member.rayon}</span>
                            <span className="flex items-center gap-1"><span className="material-symbols-outlined text-[12px] text-primary">cake</span> {calculateAge(member.birthDate)} Thn</span>
                            <span className="flex items-center gap-1"><span className="material-symbols-outlined text-[12px] text-primary">family_restroom</span> {member.familyMembers || 0} Anggota</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 overflow-y-auto custom-scrollbar p-1 pb-4 z-10 content-start flex-1 mt-4">
            ...
            {/* Actions Footer */}
            <div className="flex flex-col sm:flex-row justify-between gap-3 pt-6 pb-2 sticky bottom-0 bg-white/95 dark:bg-slate-950/95 backdrop-blur-md px-1 mt-auto border-t border-slate-100 dark:border-slate-800 shrink-0 z-20 shadow-[0_-10px_10px_-5px_rgba(255,255,255,1)] dark:shadow-none">
                <button
