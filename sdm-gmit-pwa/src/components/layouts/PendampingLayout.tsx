...
interface PendampingLayoutProps {
    children: React.ReactNode;
    title?: string;
}

export const PendampingLayout = ({ children, title = "Dashboard" }: PendampingLayoutProps) => {
    ...
    return (
        <div className="font-display bg-background-light dark:bg-slate-950 text-slate-900 dark:text-slate-100 min-h-screen flex overflow-hidden selection:bg-blue-500/30">
            {/* Sidebar */}
            <aside className={`border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex flex-col shrink-0 transition-all duration-300 fixed md:static z-40 h-full ${sidebarOpen ? 'translate-x-0 w-64' : '-translate-x-full w-64 md:w-0 md:border-r-0 md:translate-x-0 md:overflow-hidden'}`}>
                ...
                <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center gap-3">
                    <div className="size-10 bg-blue-500 rounded-xl shadow-lg shadow-blue-500/20 flex items-center justify-center text-white">
                        <span className="material-symbols-outlined">diversity_3</span>
                    </div>
                    <div>
                        <h1 className="text-sm font-bold leading-none text-slate-900 dark:text-white">Pendamping</h1>
                        ...
                        <div className="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl flex items-center justify-between group cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors" onClick={handleLogout}>
                            <div className="flex items-center gap-3">
                                ...
                                <div className="overflow-hidden">
                                    <p className="text-xs font-bold truncate text-slate-900 dark:text-white">Pendamping</p>
